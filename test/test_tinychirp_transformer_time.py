...
if __name__ == "__main__":
    x = torch.randn(*TinyChirpTransformerTime.DEFAULT_INPUT_SHAPE[:-1], 1)
    ori_mod = TinyChirpTransformerTime().eval()
    graph_analyser = GraphAnalyser(ori_mod, TinyChirpTransformerTime.DEFAULT_INPUT_SHAPE, TinyChirpTransformerTime.DEFAULT_SLIDING_STEP_SIZE)
    ...
    x = torch.randn(*TinyChirpTransformerTime.DEFAULT_INPUT_SHAPE)
